...
ss::future<> group_manager::handle_partition_leader_change(
  ...
  std::optional<model::node_id> leader_id) {
    ...
    if (leader_id == _self.id()) {
        ...
        return _catchup_lock.take_writer_lock().then([this, term, timeout, p] { 
            return inject_noop(p->partition, timeout).then([this, term, timeout, p] {
                /*
              ...
              return p->partition->make_reader(reader_config)
                .then([this, term, p, timeout](model::record_batch_reader reader) {
                    return std::move(reader)
                      .consume(recovery_batch_consumer(p->as), timeout)
                      .then([this, term, p](recovery_batch_consumer_state state) {
  ...
  ss::lw_shared_ptr<attached_partition> p, recovery_batch_consumer_state ctx) {
    ...
    for (auto& [group_id, group_stm] : ctx.groups) {
        if (group_stm.has_data()) {
            ...
            for (auto& [key, meta] : group_stm._offsets) {
    ...
    for (auto& [group_id, group_stm] : ctx.groups) {
        if (group_stm.is_removed()) {
            if (_groups.contains(group_id) && group_stm._offsets.size() > 0) {
                return ss::make_exception_future<>(std::runtime_error("unexpected unload of active group"));
            }
        }
    }
...
recovery_batch_consumer::operator()(model::record_batch batch) {
    ...
    if (batch.header().type == raft::data_batch_type) {
        ...
        auto val = reflection::from_iobuf<group_log_prepared_tx>(std::move(val_buf));
...
ss::future<> recovery_batch_consumer::handle_offset_metadata(
  iobuf key_buf, std::optional<iobuf> val_buf) {
    ...
    if (val_buf) {
        ...
        group_it->second.update_offset(key, batch_base_offset, std::move(metadata));
    } else {
        // tombstone
