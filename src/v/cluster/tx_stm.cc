...
static iobuf serialize_prepare(tx_stm::prepare_marker record) {
    iobuf b;
    kafka::response_writer w(b);
...
static tx_stm::prepare_marker deserialize_prepare(iobuf buf) {
    kafka::request_reader buf_reader(std::move(buf));
    auto version = model::control_record_version(buf_reader.read_int16());
    vassert(version == tx_stm::prepare_control_record_version, "unknown prepare record version");
    return tx_stm::prepare_marker {
...
static model::record_batch make_prepare_control_record_batch(tx_stm::prepare_marker record) {
    auto header = make_control_header(tx_prepare_batch_type, record.pid);
    header.record_count = 1;

    auto key = serialize_prepare(record);
    iobuf records;
    model::append_record_to_buffer(
        records,
        make_control_record(std::move(key)));
...
ss::future<tx_errc>
tx_stm::prepare_tx(model::term_id etag, model::partition_id tm, model::producer_identity pid, model::tx_seq tx_seq, [[maybe_unused]] model::timeout_clock::time_point timeout) {
    auto is_ready = co_await sync(2'000ms);
    if (!is_ready) {
        co_return tx_errc::timeout;
    }
    ...
    auto batch = make_prepare_control_record_batch(prepare_marker {
        .tm_partition = tm,
        .tx_seq = tx_seq,
        .pid = pid
    });
...
ss::future<tx_errc>
tx_stm::commit_tx(model::producer_identity pid, model::tx_seq tx_seq, [[maybe_unused]] model::timeout_clock::time_point timeout) {
    // <fencing>
    ...
    if (_mem_state.has_prepare_applied.contains(pid)) {
        // can't commit while there is an inflight prepare
        // possibility for the false commit the next prepare
        // during recommit recovery see tm_stm
        // todo: logging
        co_return tx_errc::timeout;
    }
    ...
    if (prepare_it->second.tx_seq > tx_seq) {
        // recommit from recovery, skipping the commit
        // to avoid false commit of the next prepare (prepare_it)
        co_return tx_errc::success;
    }

    if (prepare_it->second.tx_seq < tx_seq) {
        // violation: vassert, hang, ignore
        co_return tx_errc::success;
    }
...
void tx_stm::replay(model::record_batch&& b) {
    ...
    if (hdr.type == tx_prepare_batch_type) {
        vassert(b.record_count() == 1, "prepare batch must contain a single record");
        auto r = b.copy_records();
        auto& record = *r.begin();
        auto prepare = deserialize_prepare(record.release_key());
        prepare.pid = bid.pid;
        ...
        // recovery (see tm_stm)
        if (!_log_state.prepared.contains(pid)) {
            _log_state.prepared.emplace(pid, prepare);
        }
        _mem_state.expected.erase(pid);
