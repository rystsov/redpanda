...
tm_stm::tm_stm(
  ...
  : persisted_stm<tm_snapshot, tm_snapshot_header>(logger, c, config)
{}
...
void tm_stm::load_snapshot(tm_snapshot&) {

}

ss::future<>
tm_stm::start() {
    return persisted_stm<tm_snapshot, tm_snapshot_header>::start();
}

tm_snapshot tm_stm::take_snapshot() {
    auto snapshot = tm_snapshot();
    snapshot.offset = _insync_offset;
    return tm_snapshot();
}
